 ...
 * group PerfMark recorded tasks and links together.  This allows dynamically enabling and
 * disabling PerfMark.   Each time the library is enabled, a new generation is started and
 * associated with all the recorded data.
 *
...
public abstract class Generator {
  ...
  /**
   * Represents a failure to enable PerfMark library.  This can be used by subclasses to indicate
   * a previous failure to set the generation.  It can also be used to indicate the generation count
   * has overflowed.
   */
  ...
  /**
   * Sets the current generation count.  This should be eventually noticeable for callers of
   * {@link #getGeneration()}.  An odd number means the library is enabled, while an even number
   * means the library is disabled.
   *
